...
router.post('/:id/entries', async (req, res) => {
  try {
    ...
    if (entryError) {
      ...
      // Handle duplicate entry (user already bet on this prediction/option)
      if (entryError.code === '23505' && entryError.message.includes('prediction_entries_prediction_id_user_id_option_id_key')) {
        return res.status(409).json({
          error: 'Duplicate entry',
          message: 'You have already placed a bet on this option for this prediction',
          version: VERSION
        });
      }

...
export const ClubDetailPage: React.FC<{ clubId?: string }> = ({ clubId: propClubId }) => {
  ...
  const fetchClubDetails = async () => {
    try {
      setLoading(true)
      console.log('🔍 ClubDetailPage: Fetching club details for ID:', clubId)
      
      // Fetch club details
      const clubResponse = await fetch(`/api/clubs/${clubId}`)
      console.log('🔍 ClubDetailPage: Club response status:', clubResponse.status)
      
      if (clubResponse.ok) {
        const clubData = await clubResponse.json()
        console.log('🔍 ClubDetailPage: Club data received:', clubData)
        setClub(clubData.data?.club || clubData.club)
      } else {
        console.error('🔍 ClubDetailPage: Club response not ok:', clubResponse.status, clubResponse.statusText)
      }

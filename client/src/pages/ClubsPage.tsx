...
// Component that wraps ClubDetailPage with mock data for development
const ClubDetailPageWithData: React.FC<{ clubData: Club; onBack: () => void }> = ({ clubData, onBack }) => {
  const { setCurrentClub, currentClub, joinClub, leaveClub } = useClubStore();
  const { user } = useAuthStore();
  
  // Convert basic club data to ExtendedClub format
  React.useEffect(() => {
    console.log('Setting up club data for:', clubData.name);
    const extendedClub = {
      ...clubData,
      id: clubData.id,
      name: clubData.name,
      ...
      activePredictions: Math.floor(Math.random() * 10) + 5,
      stats: {
        totalPredictions: Math.floor(Math.random() * 100) + 20,
        correctPredictions: Math.floor(Math.random() * 50) + 10,
        totalWinnings: Math.floor(Math.random() * 100000) + 5000,
        topMembers: Math.floor(Math.random() * 20) + 5,
      },
    };

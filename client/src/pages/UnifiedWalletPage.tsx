...
import { Plus, ArrowDownToLine, Lock, Wallet, RefreshCw, HelpCircle } from 'lucide-react';
...
const WalletPage: React.FC<WalletPageProps> = ({ onNavigateBack }) => {
  ...
  const getActivityIcon = (kind: string) => {
    switch (kind) {
      ...
      case 'payout':
        return { Icon: ZaurumMark, color: 'green' };
      case 'creator_fee':
        return { Icon: ZaurumMark, color: 'emerald' };
      case 'platform_fee':
        return { Icon: ZaurumMark, color: 'indigo' };
      default:
        return { Icon: ZaurumMark, color: 'gray' };
    }
  };
  ...
  return (
    <>
      ...
      {/* Modals */}
      {showDepositModal && user?.id && !isZaurumOnly && (
        <DepositUSDCModal
      ...
      {showWithdrawModal && user?.id && !isZaurumOnly && (
        <WithdrawUSDCModal
      ...
      {showConnectWallet && !isZaurumOnly && (
        <ConnectWalletSheet

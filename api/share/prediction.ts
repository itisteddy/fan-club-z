...
function resolveApiBase(req: any): string {
  const host = String((req.headers['x-forwarded-host'] as string) || (req.headers.host as string) || '').toLowerCase();
  if (host.includes('staging')) return 'https://fanclubz-backend-staging.onrender.com';

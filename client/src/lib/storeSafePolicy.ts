...
export function getBlockedFeatureMessage(featureKey: 'crypto-wallet' | 'fiat-payments' | 'withdrawals'): string {
  ...
  switch (featureKey) {
    case 'crypto-wallet':
      return 'Crypto wallet features are not available in Zaurum mode.';
    case 'fiat-payments':
      return 'Fiat payments are not available in Zaurum mode.';
    case 'withdrawals':
      return 'Withdrawals are not available in Zaurum mode.';
    default:

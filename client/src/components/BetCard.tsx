...
const BetCard: React.FC<BetCardProps> = ({
  ...
  if (variant === 'user-entry') {
    return (
      <motion.div
        className="bg-white rounded-2xl shadow-sm border border-gray-100/50 p-6 mb-4"
        style={{
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="bg-gray-50/80 rounded-xl p-4">
            ...
            <div className="text-xl font-bold text-gray-900">₦{(bet.stakeMin * 2).toLocaleString()}</div>
          </div>
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4">
            <div className="text-sm text-gray-600 mb-1">
              {bet.status === 'settled' ? 'Final Return' : 'Potential Return'}
            </div>
            <div className="text-xl font-bold text-green-700">₦{(bet.stakeMin * 3.5).toLocaleString()}</div>
            <div className="text-sm text-green-600 font-medium">+75% gain</div>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <motion.button
            onClick={onBet}
            className="flex-1 bg-gradient-to-r from-green-600 to-emerald-600 text-white py-3 rounded-xl font-medium"

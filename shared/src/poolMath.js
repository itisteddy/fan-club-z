...
exports.getPreOddsMultiple = getPreOddsMultiple;
exports.getPostOddsMultiple = getPostOddsMultiple;
exports.getPayoutPreview = getPayoutPreview;
const BPS_DENOM = 10000;
/**
 * Current odds (before user's stake): T / Wi.
 * Returns null if optionPool <= 0 (guard division by zero).
 */
function getPreOddsMultiple(totalPool, optionPool) {
 ...
 */
function getPostOddsMultiple(input) {
 ...
 */
function getPayoutPreview(input) {
    const { totalPool, optionPool, stake, feeBps } = input;
    const T = Number(totalPool) || 0;
    const Wi = Number(optionPool) || 0;
    const s = Number(stake) || 0;

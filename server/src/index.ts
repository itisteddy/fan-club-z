...
app.options('*', (req, res) => {
  ...
  res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, Accept, Origin, X-Requested-With, Cache-Control, If-None-Match, x-fcz-client, X-FCZ-Client');
  res.sendStatus(200);
});
...
console.log('  - /api/v2/moderation (blocking/reporting)');
console.log('  - /api/v2/content/report (content reporting)');
...
console.log('  - /api/demo-wallet (daily claim & demo balances)');

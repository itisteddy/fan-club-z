...
// Validation schemas
const settleManualSchema = z.object({
...
const resolveDisputeSchema = z.object({
  ...
  resolutionReason: z.string().min(10).max(500),
  newWinningOptionId: z.string().uuid().optional()
});

// Settlement endpoints
router.post('/settle-manual', authenticateToken, async (req, res) => {
  try {
    ...
    const { data: settlement, error: settlementError } = await supabase.rpc('settle_prediction_manual', {
...
router.post('/settle-auto', authenticateToken, async (req, res) => {
  try {
    ...
    const { data: settlement, error } = await supabase.rpc('settle_prediction_auto', {
      p_prediction_id: predictionId,
      p_winning_option_id: winningOptionId,
...
router.post('/resolve-dispute', authenticateToken, async (req, res) => {
  try {
    ...
    if (resolution === 'approved' && newWinningOptionId) {
      ...
      const { error: resettleError } = await supabase.rpc('resettle_prediction', {
...
router.get('/analytics', authenticateToken, async (req, res) => {
  try {
    ...
    const { data: analytics, error } = await supabase.rpc('get_settlement_analytics');
...
router.get('/criteria-templates', authenticateToken, async (req, res) => {
  try {
...
router.get('/verification-sources', authenticateToken, async (req, res) => {
  try {
    const { category } = req.query;
...
router.post('/blockchain-transaction', authenticateToken, async (req, res) => {
  try {

...
const BetsTab: React.FC = () => {
  ...
  const EmptyState = ({ tab }) => (
    <div className="flex flex-col items-center justify-center py-16 px-6">
      ...
      <button className="bg-emerald-500 hover:bg-emerald-600 text-white px-8 py-3 rounded-xl font-medium transition-all duration-200 transform hover:scale-105 flex items-center gap-2"
        onClick={() => setLocation(tab === 'Created' ? '/create' : '/discover')}
      >
  ...
  const CreatedPredictionCard = ({ prediction }) => (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-4 hover:shadow-md transition-all duration-200">
      ...
      {/* Footer */}
      <div className="flex items-center justify-between text-sm text-gray-500">
        <span>Closes in {prediction.timeRemaining}</span>
        <button className="text-blue-600 font-medium flex items-center gap-1 hover:text-blue-700">

...
router.get('/test', (req, res) => {
    logger_1.default.info('Comment test route hit');
    res.json({
    ...
    try {
        ...
        `)
            ...
            if (!error && comments) {
                ...
                logger_1.default.info(`Successfully fetched ${formattedComments.length} comments from Supabase`);
                return res.json({
            ...
            logger_1.default.warn('Supabase not available, using in-memory storage:', dbError);
        }
        ...
        logger_1.default.info(`Successfully returning ${comments.length} persistent comments`);
        res.json({
            comments,
            hasMore: false,
        ...
        logger_1.default.error('Error in get comments route:', error);
        res.status(500).json({
            success: false,
...
router.post('/predictions/:predictionId/comments', async (req, res) => {
    try {
        ...
        logger_1.default.info(`Creating comment for prediction ${predictionId}`);
        logger_1.default.info('Request body:', JSON.stringify(req.body));
        // Basic validation
        if (!content || typeof content !== 'string' || content.trim().length === 0) {
            logger_1.default.warn('Invalid content provided:', content);
        ...
        if (content.length > 500) {
            logger_1.default.warn('Content too long:', content.length);
            return res.status(400).json({
                success: false,
        ...
        // Try Supabase first
        try {
            const { data: insertedComment, error } = await supabase_1.supabase
                .from('comments')
                .insert({
                id: newComment.id,
            ...
            if (!error && insertedComment) {
                logger_1.default.info(`Comment saved to Supabase: ${insertedComment.id}`);
        ...
        logger_1.default.info(`Comment persisted in memory: ${newComment.id}`);
        logger_1.default.info(`Total comments for ${predictionId}: ${commentsStorage[predictionId].length}`);
        res.status(201).json(newComment);
    }
    catch (error) {
        logger_1.default.error('Error in create comment route:', error);
        res.status(500).json({
            success: false,
            error: 'Internal server error',
            details: error instanceof Error ? error.message : 'Unknown error'
...
router.post('/comments/:commentId/like', async (req, res) => {
    try {
        ...
        logger_1.default.info(`Like toggled for comment ${commentId}`);
        // Try Supabase first
        try {
            ...
            logger_1.default.warn('Supabase not available for likes:', dbError);
        }
        // Return default values on database error
        const liked = false;
        const likes_count = 0;
        logger_1.default.info(`Like result: liked=${liked}, likes_count=${likes_count}`);
        res.json({
            success: true,
            liked,
            likes_count,
        ...
        logger_1.default.error('Error in like comment route:', error);
        res.status(500).json({
            success: false,
            error: 'Internal server error',
            details: error instanceof Error ? error.message : 'Unknown error'
...
router.put('/comments/:commentId', async (req, res) => {
    try {
        ...
        logger_1.default.info(`Editing comment ${commentId} with content: ${content}`);
        // Basic validation
        if (!content || typeof content !== 'string' || content.trim().length === 0) {
            return res.status(400).json({
                success: false,
        ...
        // Try Supabase first
        try {
            const { data: updatedComment, error } = await supabase_1.supabase
                .from('comments')
                .update({
                content: content.trim(),
                updated_at: new Date().toISOString(),
            ...
            logger_1.default.warn('Supabase not available for edit:', dbError);
        }
        ...
        logger_1.default.error('Error in edit comment route:', error);
        res.status(500).json({
            success: false,
            error: 'Internal server error',
            details: error instanceof Error ? error.message : 'Unknown error'
...
router.delete('/comments/:commentId', async (req, res) => {
    try {
        ...
        try {
            ...
            logger_1.default.warn('Supabase not available for delete:', dbError);
        }
        ...
        logger_1.default.error('Error in delete comment route:', error);
        res.status(500).json({
            success: false,
            error: 'Internal server error',
            details: error instanceof Error ? error.message : 'Unknown error'
...
router.all('*', (req, res) => {
    logger_1.default.warn(`Unhandled comment route: ${req.method} ${req.originalUrl}`);
    logger_1.default.warn('Available routes:');
    logger_1.default.warn('- GET /test');
    logger_1.default.warn('- GET /health');
    logger_1.default.warn('- GET /predictions/:predictionId/comments');
    logger_1.default.warn('- POST /predictions/:predictionId/comments');
    logger_1.default.warn('- POST /comments/:commentId/like');
    logger_1.default.warn('- PUT /comments/:commentId');
    logger_1.default.warn('- DELETE /comments/:commentId');
    logger_1.default.warn('- GET /debug/storage');
    res.status(404).json({
        success: false,

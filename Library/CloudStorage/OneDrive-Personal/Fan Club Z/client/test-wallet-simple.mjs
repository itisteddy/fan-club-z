...
async function testWalletSimple() {
  ...
  try {
    console.log('🔍 Testing wallet functionality with direct navigation...');
    ...
    if (walletUrl.includes('/wallet')) {
      ...
      if (headerExists > 0) {
        ...
        if (balanceExists > 0) {
          ...
          // Test the 8 specific features
          console.log('\n📋 Testing the 8 wallet features:');
          
          // 1. Display wallet balance
          console.log('1. ✅ Display wallet balance - ' + balanceText);
          
          // 2. Show quick deposit amounts
          console.log('2. ✅ Show quick deposit amounts - ' + quickDepositCount + ' buttons found');
          
          // 3. Allow custom deposit amounts
          const addFundsButton = await page.locator('button:has-text("Add Funds")');
          const addFundsExists = await addFundsButton.count();
          console.log('3. ✅ Allow custom deposit amounts - Add Funds button found:', addFundsExists > 0);
          
          // 4. Show transaction history
          console.log('4. ✅ Show transaction history - ' + transactionCount + ' items found');
          
          // 5. Allow withdrawals
          const withdrawButton = await page.locator('button:has-text("Withdraw")');
          const withdrawExists = await withdrawButton.count();
          console.log('5. ✅ Allow withdrawals - Withdraw button found:', withdrawExists > 0);
          
          // 6. Update wallet balance after placing bet (this would need a separate test)
          console.log('6. ⏳ Update wallet balance after placing bet - Would need bet placement test');
          
          // 7. Navigate to wallet after demo login
          console.log('7. ✅ Navigate to wallet after demo login - Successfully navigated');
          
          // 8. Display wallet balance correctly
          console.log('8. ✅ Display wallet balance correctly - ' + balanceText + ' displayed');
    ...
    // Take a screenshot
    await page.screenshot({ path: 'test-wallet-simple.png', fullPage: true });
    console.log('📸 Screenshot saved as test-wallet-simple.png');
    
  } catch (error) {
    console.error('❌ Test failed:', error);
    await page.screenshot({ path: 'test-wallet-simple-error.png', fullPage: true });
  } finally {
    await browser.close();
  }
}

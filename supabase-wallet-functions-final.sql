-- Fan Club Z Wallet Functions for Supabase (FIXED AMBIGUITY VERSION)
-- Run this SQL in your Supabase SQL Editor

-- Drop existing function if it exists
DROP FUNCTION IF EXISTS update_wallet_balance(UUID, TEXT, DECIMAL, DECIMAL);

-- Create the update_wallet_balance RPC function with fully qualified table references
...
AS $$
BEGIN
  -- Create wallet if it doesn't exist (fully qualified column references)
  INSERT INTO wallets (wallets.user_id, wallets.currency, wallets.available_balance, wallets.reserved_balance)
  VALUES (p_user_id, p_currency_code, GREATEST(0, p_available_change), GREATEST(0, p_reserved_change))
  ON CONFLICT (wallets.user_id, wallets.currency) DO NOTHING;
...
-- Test the function to make sure it works (simplified test)
DO $$
DECLARE
  test_user_id UUID := '00000000-0000-0000-0000-000000000001'::UUID;
BEGIN
  -- Simple test that doesn't depend on existing users
  RAISE NOTICE 'Wallet functions created successfully! Test with user ID: %', test_user_id;
END;
$$;

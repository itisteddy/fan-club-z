-- Fan Club Z - Prediction Placement Fix Migration (USD VERSION)
-- Run this SQL in your Supabase SQL Editor to fix prediction placement issues

-- 1. First, ensure USD is the default currency (keep existing USD settings)
-- This script maintains USD as the primary currency

-- 2. Create USD wallets for users who don't have them (keep existing USD wallets)
INSERT INTO wallets (user_id, currency, available_balance, reserved_balance)
SELECT u.id, 'USD', 25000.00, 0.00
FROM users u
WHERE u.id NOT IN (SELECT w.user_id FROM wallets w WHERE w.currency = 'USD')
ON CONFLICT (user_id, currency) DO NOTHING;

-- 3. Add participant_count column to predictions table if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT column_name 
...
-- 4. Update participant_count for existing predictions
UPDATE predictions 
...
-- 5. Fix transaction types - add prediction_lock if not exists
DO $$
BEGIN
  BEGIN
...
-- 6. Create index on prediction_entries for better performance
CREATE INDEX IF NOT EXISTS prediction_entries_prediction_user_idx 
ON prediction_entries(prediction_id, user_id);

-- 7. Create helper function to recalculate prediction odds
...
-- 8. Recalculate odds for all active predictions
DO $$
DECLARE
  pred_record RECORD;
BEGIN
...
-- 9. Ensure USD is the default currency for new records
DO $$
BEGIN
  BEGIN
...
-- 10. Verify the setup by checking a few things
DO $$
DECLARE
  wallet_count INTEGER;
  usd_wallet_count INTEGER;
  function_exists BOOLEAN;
BEGIN
  -- Check wallet counts
  SELECT COUNT(*) INTO wallet_count FROM wallets;
  SELECT COUNT(*) INTO usd_wallet_count FROM wallets WHERE currency = 'USD';
  ...
  RAISE NOTICE 'Total wallets: %, USD wallets: %', wallet_count, usd_wallet_count;
  RAISE NOTICE 'update_wallet_balance function exists: %', function_exists;
END $$;

-- Success message
SELECT 'Prediction placement fix migration completed successfully with USD currency!' AS status;

  ...
  try {
    ...
    const test5 = await testFeature('Bottom navigation tabs are present', async () => {
      ...
      for (const tab of tabs) {
        await page.waitForSelector(`button:has-text("${tab}")`, { timeout: 5000 });
      }
    });
    results.total++;
    ...
    // Test 6: Navigation to My Bets tab
    const test6 = await testFeature('Navigation to My Bets tab', async () => {
